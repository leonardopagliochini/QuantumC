...
def main():
    ...
    vals = [-2, -1, 0, 1, 2]
    divisors = [-2, -1, 1, 2]
    print("divi_controlled: testing a in -2..2 and divisor in {-2,-1,1,2}, control=1")
    for a in vals:
        for d in divisors:
            print(f"divi_controlled: a={a}, divisor={d}")
            ...
            rows.append(("divi_controlled", a, a_bin, d, d_bin, exp_q, exp_q_bin, exp_r, exp_r_bin, q_val, q_bits, r_val, r_bits, (q_val == exp_q) and (r_val == exp_r)))
    tu.print_table(rows, csv_path="test_log/test_divi_controlled.csv", headers=[
        "op","a_dec","a_bin","d_dec","d_bin","exp_q_dec","exp_q_bin","exp_r_dec","exp_r_bin","meas_q_dec","meas_q_bin","meas_r_dec","meas_r_bin","ok"])
