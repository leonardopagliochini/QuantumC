...
def main():
    ...
    rows = []
    vals = [-2, -1, 0, 1, 2]
    print("add_controlled: testing a and b in range -2..2, control in {0,1}")
    for ctrl_val in [0, 1]:
        for a in vals:
            for b in vals:
                print(f"add_controlled: a={a}, b={b}, control={ctrl_val}")
                ...
                rows.append(("add_controlled", ctrl_val, a, a_bin, b, b_bin, exp, exp_bin, res, res_bits, res == exp))
    tu.print_table(
        rows,
        csv_path="test_log/test_add_controlled.csv",
        headers=[
            "op",
            "ctrl",
            "a_dec",
