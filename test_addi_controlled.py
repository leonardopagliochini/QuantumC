...
def main():
    ...
    rows = []
    vals = [-2, -1, 0, 1, 2]
    print("addi_controlled: testing a and b in range -2..2, control=1")
    for a in vals:
        for b in vals:
            print(f"addi_controlled: a={a}, b={b}")
            ...
            rows.append(("addi_controlled", a, a_bin, b, b_bin, exp, exp_bin, res, res_bits, res == exp))
    tu.print_table(rows, csv_path="test_log/test_addi_controlled.csv")
