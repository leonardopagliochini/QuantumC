...
- Generate a CC×k grid without loops:
    python benchmarking/bench_tools/gen_cc_ir_programs.py gen-grid \
...
Notes:
- This script emits only C; scanning/measurement is handled by benchmarking/tools_old/scan_cc_ir.py.
"""
...
def make_prework(steps: int, ops_per_iter: int) -> str:
    ...
    for t in range(int(steps)):
        ...
        for i in range(max(1, ops_per_iter)):
            seq.append(PREWORK_CORE[i % len(PREWORK_CORE)])
    return "".join(f"  {line}\n" for line in seq)


def make_prework_prefix(extra_ops: int) -> str:
    if extra_ops <= 0:
        return ""
    extra_ops = min(extra_ops, len(PREWORK_CORE))
    return "".join(f"  {line}\n" for line in PREWORK_CORE[:extra_ops])
...
def generate_c(cc_target: int, iters: int, ops_per_iter: int,
    ...
    if extra_ops > 0:
        parts.append(make_prework_prefix(extra_ops))
