#!/usr/bin/env python3
"""
Generate C corpora where each addition redistributes 'outside' budget by conditional depth.

New budget rule (requested):
- Start from a TOTAL outside budget T.
- For each addition placed inside k nested ifs, remove k additions from outside.
  (k = 1,2,3,...)

Therefore allocations (x0 outside, xk inside k-nested) must satisfy:
    x0 + 1*x1 + 2*x2 + 3*x3 + ... + K*xK = T
(where by construction K <= T; we enumerate up to K = T).

Concretely:
- We produce files for T in {2,4,6,...,30} by default.
...
def enumerate_allocations(total: int) -> List[Dict[int, int]]:
    ...
    Enumerate all nonnegative integer solutions to:
        x0*1 + sum_{k=1..K} xk*(k) = total
    with K = total (placing an add at depth k 'consumes' k from outside).

    Returns a list of allocations {0:x0, 1:x1, 2:x2, ...} (sparse dicts).
    """
    # weights: outside costs 1 per outside add; depth k costs k units of outside budget
    weights = {0: 1}
    K = total
...
def main():
    ...
    ap.add_argument(
        ...
        help="Output folder for instruction-redistribution corpora",
    )
    ap.add_argument(
        "--min-total", type=int, default=2, help="Minimum total outside budget (even)"
    )
    ap.add_argument(
        "--max-total", type=int, default=30, help="Maximum total outside budget (even)"
    )
    ap.add_argument(
    ...
    for T in totals:
        ...
        def alloc_key(alloc: Dict[int, int]):
            x0 = alloc.get(0, 0)
            vec = tuple(alloc.get(k, 0) for k in range(1, T + 1))
