...
def main():
    ...
    min_val = vals[0]
    max_val = vals[-1]
    print(f"not_equal: testing a and b in range {min_val}..{max_val}, {total} operations")
    for a in vals:
        for b in vals:
            print(f"not_equal: a={a}, b={b}")
            ...
            exp = 1 if a != b else 0
            rows.append(("not_equal", a, a_bin, b, b_bin, exp, str(exp), res, res_bits, res == exp))
    tu.print_table(rows, csv_path="test_log/test_not_equal.csv")
