...
def main():
    ...
    rows = []
    vals = [0, 1, 2, 3]
    print("divu_controlled: testing unsigned a,b in 0..3, control=1")
    for a in vals:
        for b in vals:
            if b == 0:
                continue
            print(f"divu_controlled: a={a}, b={b}")
            ...
            rows.append(("divu_controlled", a, a_bin, b, b_bin, exp_q, exp_q_bin, exp_r, exp_r_bin, q_val, q_bits, r_val, r_bits, (q_val == exp_q) and (r_val == exp_r)))
    tu.print_table(rows, csv_path="test_log/test_divu_controlled.csv", headers=[
        "op","a_dec","a_bin","b_dec","b_bin","exp_q_dec","exp_q_bin","exp_r_dec","exp_r_bin","meas_q_dec","meas_q_bin","meas_r_dec","meas_r_bin","ok"])
